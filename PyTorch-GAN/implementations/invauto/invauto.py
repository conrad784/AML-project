...
G_AB = Generator(res_blocks=opt.n_residual_blocks)
D_A = Discriminator()
D_B = Discriminator()
...
def sample_images(batches_done):
    ...
    fake_B = G_AB(real_A, type_ = 'encoder')
    rec_A = G_AB(fake_B, type_ = 'decoder')
    real_B = Variable(imgs['B'].type(Tensor))
    fake_A = G_AB(real_B, type_ = 'decoder')
    rec_B=G_AB(fake_A, type_ = 'encoder')
    img_sample = torch.cat((real_A.data, fake_B.data, rec_A.data,
                            real_B.data, fake_A.data, rec_B.data), 0)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Identity loss
        loss_id_A = criterion_identity(G_AB(real_A, type_ = 'decoder'), real_A)
        loss_id_B = criterion_identity(G_AB(real_B, type_ = 'encoder'), real_B)
        ...
        # Cycle loss
        recov_A = G_AB(fake_B, type_ = 'decoder')
